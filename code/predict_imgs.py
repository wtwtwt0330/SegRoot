...
parser.add_argument(
    ...
    default="test.jpg",
    type=str,
    help="filename of one test image")
parser.add_argument(
    "--thres",
    default=0.9,
    type=float,
    help="threshold of the final binarization")
parser.add_argument(
    "--all",
    action="store_true",
    help="make prediction on all images in the folder")
parser.add_argument(
    "--data_dir",
...
def predict_gen(model, img_path, thres, device, info):
    ...
    # reverse padding
    prediction = prediction[(NH-H) // 2 : (NH-H) // 2 + H, (NW-W) // 2 : (NW-W) // 2 + W]
...
if __name__ == '__main__':
    ...
    # define model
    print('using segnet, width : {}, depth : {}'.format(8, 5))
    model = SegRoot(8, 5).to(device)
    weights_path = Path('../weights/best_segnet-(8,5)-0.6441.pt')

    if device.type == 'cpu':
        print('load weights to cpu')
        print(weights_path.as_posix())
        model.load_state_dict(torch.load(
            weights_path.as_posix(), map_location='cpu'))
    else:
